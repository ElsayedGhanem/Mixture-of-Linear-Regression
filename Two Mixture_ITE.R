...
n0 <- c(60,100,200)
p0 <- 5   
rho0 <- c(0.88,0.90,0.95,0.97,0.99)  # correlation between predictors
phi0 <- c(0.88,0.90,0.95,0.97,0.99)  # correlation between predictors
Sigma<-c(0.5,1,2)
sigma0 <- c(Sigma[1],Sigma[1])
rho0t <- rho0[1]
phi0t <- phi0[1]
n0t <- n0[1]
...
Simulation <- function(n,p,rho,phi,epsilon,M,Beta,beta.0,beta.old,pi,pi.0,pi.old,sigma,sigma.0,sigma.old,maxit_EM,printout=F)
{
  ...
  resArray_LS_EM <-resArray_LS_CEM <- resArray_LS_SEM <- array(NA,dim=c(2,5,Nsim))
  ...
  resArray_ridge_EM <-resArray_ridge_CEM <- resArray_ridge_SEM <- array(NA,dim=c(2,5,Nsim))
  ...
  resArray_Liu_EM <-resArray_Liu_CEM <- resArray_Liu_SEM <- array(NA,dim=c(2,5,Nsim))
...
save(MSE_Result,file="mse_r1ph1n1_ite.RData")
save(pre,file="test_r1ph1n1_ite.RData")
