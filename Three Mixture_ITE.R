...
rm(list=ls())
# library(psych)
n0 <- c(60,100,200)  #(60,100)
p0 <- 3   
#rho0 <- c(0.9,0.95,0.99)  # correlation between predictors
phi0 <- c(0.90,0.92,0.95,0.97,0.99)  # correlation between predictors - Three Mixtures
# phi0 <- c(0.88,0.90,0.95,0.97,0.99)  # correlation between predictors - Two Mixtures
#rho0t <- rho0[3]
phi0t <- phi0[1]
n0t <- n0[1]
M0 <- 3 # number of mixture component
...
pi0 <- c(0.3,0.4,0.3)
Sigma<-c(0.5,1,2)
sigma0 <- c(Sigma[1],Sigma[1],Sigma[1])
#sigma0 <- c(0.25,1,0.09)
maxit_em0 <- 200
epsilon_em0 <- 1e-5
k_fold <- 5
CN0 <- 2000        ##{500,2000}
Nsim <- 200         ##{200,500,2000}
...
pi.01 <- c(0.3,0.4,0.3)
pi.02 <- c(0.3,0.4,0.3)
sigma.01 <- sigma0+0.02
sigma.02 <- sigma0+0.02
...
## phi: correlation between 2 predictors
...
canolical <- function(x,y,w,sig2hat)
{
  np <- dim(x)
  n  <- np[1]
  p  <- np[2]
...
em_Liu <-function(epsilon,x,y,beta.0,beta.old,pi.0,pi.old,sigma.0,sigma.old,maxit_EM,Sigma_Ridge_EM,printout=F)
{
  pp <- dim(x)[2]
  M <- length(pi.0)  
  ...
  for(k in 1:maxit_EM)
  {
    ...
    for (j in 1:ncol(Z)) 
    {
      parameter <- canolical(x,y,tau[,j],Sigma_Ridge_EM[j])
      k_j <- parameter$k
      d_j <- parameter$d
      beta_Liu <- update_Beta_Liu(x,y,tau[,j],beta.k[j,],beta.old[j,],k_j,d_j)
      #beta_mle <- solve(t(x)%*%diag(tau[,j])%*%x)%*%(t(x)%*%diag(tau[,j])%*%y)
      Beta_Liu[j,] <- beta_Liu
...
CEM_Liu <-function(epsilon,x,y,beta.0,beta.old,pi.0,pi.old,sigma.0,sigma.old,maxit_EM,Sigma_Ridge_CEM,printout=F)
{
  pp <- dim(x)[2]
  M <- length(pi.0)  
  Beta_Liu <- matrix(data=NA, nrow=M, ncol=pp)
  ...
  for(k in 1:maxit_EM)
  {
    ...
    for (j in 1:ncol(Z)) 
    {
      ...
      parameter <- canolical(x=x[zInd,],y=y[zInd],tau[zInd,j],Sigma_Ridge_CEM[j])
      k_j <- parameter$k
      d_j <- parameter$d
      beta_Liu <- update_Beta_Liu(x=x[zInd,],y=y[zInd],tau=tau[zInd,j],beta.k[j,],beta.old[j,],k_j,d_j)
      Beta_Liu[j,] <- beta_Liu
...
SEM_Liu <-function(epsilon,x,y,beta.0,beta.old,pi.0,pi.old,sigma.0,sigma.old,maxit_EM,Sigma_Ridge_SEM,printout=F)
{
  pp <- dim(x)[2]
  M <- length(pi.0)  
  Beta_Liu <- matrix(data=NA, nrow=M, ncol=pp)
  ...
  for(k in 1:maxit_EM)
  {
    ...
    for (j in 1:ncol(Z)) 
    {
      ...
      parameter <- canolical(x=x[zInd,],y=y[zInd],tau[zInd,j],Sigma_Ridge_SEM[j])
      k_j <- parameter$k
      d_j <- parameter$d
      beta_Liu <- update_Beta_Liu(x=x[zInd,],y=y[zInd],tau=tau[zInd,j],beta.k[j,],beta.old[j,],k_j,d_j)
      Beta_Liu[j,] <- beta_Liu
...
.mseBeta <- function(Mhat,Btrue)
{
  ...
  dif3 <- sqrt(dif2/(j*p))
  meanBeta <- round(mean(dif3),3)
  quanBeta <- round(quantile(dif3, probs=c(0.025,0.5,0.975)),3)
...
{
  dif1 <- ((t(P_hat)-P_true)^2)
  dif2 <- colSums(dif1)
  dif3 <- sqrt(dif2/2)
  mean_pi <- round(mean(dif3),3)
  quan_pi <- round(quantile(dif3, probs=c(0.025,0.5,0.975)),3)
...
.msesigma <- function(sigma_hat,sigma_true)
{
  ...
  dif2 <- sqrt(colSums(dif1))
  mean_sigma <- round(mean(dif2),3)
  quan_sigma <- round(quantile(dif2, probs=c(0.025,0.5,0.975)),3)
...
Simulation <- function(n,p,phi,epsilon,M,Beta,beta.0,beta.old,pi,pi.0,pi.old,sigma,sigma.0,sigma.old,maxit_EM,printout=F)
{
  ## intialization for prediction (p+1) means the number of indep variables + response variable
  
  Data <- array(NA,dim=c(n,p+1,Nsim))
  
  ## intialization for ML Method
  resArray_LS_EM <-resArray_LS_CEM <- resArray_LS_SEM <- array(NA,dim=c(M,p,Nsim))
  ...
  resArray_ridge_EM <-resArray_ridge_CEM <- resArray_ridge_SEM <- array(NA,dim=c(M,p,Nsim))
  ...
  resArray_Liu_EM <-resArray_Liu_CEM <- resArray_Liu_SEM <- array(NA,dim=c(M,p,Nsim))
  ...
  for (j in 1:Nsim)
  {
    ...
    Yres <- Design_Response_Mixture(x=x,beta=Beta,pi=pi,M=M,sigma=sigma)
    y <- Yres[,1]
    ...
    EM_Result_Liu <- em_Liu(epsilon=epsilon,x=x,y=y,beta.0=beta.0,
                                beta.old=beta.old,pi.0=pi.0,pi.old=pi.old,sigma.0=sigma.0,
                                sigma.old=sigma.old,maxit_EM=maxit_EM,Sigma_Ridge_EM,printout=F)
    ...
    CEM_Result_Liu <- CEM_Liu(epsilon=epsilon,x=x,y=y,beta.0=beta.0,
                            beta.old=beta.old,pi.0=pi.0,pi.old=pi.old,sigma.0=sigma.0,
                            sigma.old=sigma.old,maxit_EM=maxit_EM,Sigma_Ridge_CEM,printout=F)
    ...
    SEM_Result_Liu <- SEM_Liu(epsilon=epsilon,x=x,y=y,beta.0=beta.0,
                              beta.old=beta.old,pi.0=pi.0,pi.old=pi.old,sigma.0=sigma.0,
                              sigma.old=sigma.old,maxit_EM=maxit_EM,Sigma_Ridge_SEM,printout=F)
                     ...
                     ,B_true,Pi_true,sigma_true)
{
  ...
  MSE_LS_Pi_EM     <- .msepi(P_hat =  Pi_LS_EM[,1:2],P_true = Pi_true[1:2])
  ...
  MSE_Ridge_Pi_EM     <- .msepi(P_hat =  Pi_Ridge_EM[,1:2],P_true = Pi_true[1:2])
  ...
  MSE_Liu_Pi_EM     <- .msepi(P_hat =  Pi_Liu_EM[,1:2],P_true = Pi_true[1:2])
  ...
  MSE_LS_Pi_CEM     <- .msepi(P_hat =  Pi_LS_CEM[,1:2],P_true = Pi_true[1:2])
  ...
  MSE_Ridge_Pi_CEM     <- .msepi(P_hat =  Pi_Ridge_CEM[,1:2],P_true = Pi_true[1:2])
  ...
  MSE_Liu_Pi_CEM     <- .msepi(P_hat =  Pi_Liu_CEM[,1:2],P_true = Pi_true[1:2])
  ...
  MSE_LS_Pi_SEM     <- .msepi(P_hat =  Pi_LS_SEM[,1:2],P_true = Pi_true[1:2])
  ...
  MSE_Ridge_Pi_SEM     <- .msepi(P_hat =  Pi_Ridge_SEM[,1:2],P_true = Pi_true[1:2])
  ...
  MSE_Liu_Pi_SEM     <- .msepi(P_hat =  Pi_Liu_SEM[,1:2],P_true = Pi_true[1:2])
  ...
  Bias_LS_Pi_EM     <- .biaspi(P_hat =  Pi_LS_EM[,1:2],P_true = Pi_true[1:2])
  ...
  Bias_Ridge_Pi_EM     <- .biaspi(P_hat =  Pi_Ridge_EM[,1:2],P_true = Pi_true[1:2])
  ...
  Bias_Liu_Pi_EM     <- .biaspi(P_hat =  Pi_Liu_EM[,1:2],P_true = Pi_true[1:2])
  ...
  Bias_LS_Pi_CEM     <- .biaspi(P_hat =  Pi_LS_CEM[,1:2],P_true = Pi_true[1:2])
  ...
  Bias_Ridge_Pi_CEM     <- .biaspi(P_hat =  Pi_Ridge_CEM[,1:2],P_true = Pi_true[1:2])
  ...
  Bias_Liu_Pi_CEM     <- .biaspi(P_hat =  Pi_Liu_CEM[,1:2],P_true = Pi_true[1:2])
...
save(MSE_Result,file="mse_r1n1.RData")
save(pre,file="test_r1n1.RData")
