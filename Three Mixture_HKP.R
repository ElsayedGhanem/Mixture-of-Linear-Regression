...
phi0 <- c(0.9,0.92,0.95,0.97,0.99)  # correlation between predictors
#rho0t <- rho0[3]
phi0t <- phi0[1]
n0t <- n0[1]
...
em_Liu <-function(epsilon,x,y,beta.0,beta.old,pi.0,pi.old,sigma.0,sigma.old,maxit_EM,k_Liu,Sigma_Ridge_EM,printout=F)
{
  ...
  for(k in 1:maxit_EM)
  {
    ...
    for (j in 1:ncol(Z)) 
    {
      ...
      #d_j <- canolical(x,y,tau[,j],k_Liu[j],Sigma_Ridge_EM[j])
...
CEM_Liu <-function(epsilon,x,y,beta.0,beta.old,pi.0,pi.old,sigma.0,sigma.old,maxit_EM,k_Liu,Sigma_Ridge_CEM,printout=F)
{
  ...
  for(k in 1:maxit_EM)
  {
    ...
    for (j in 1:ncol(Z)) 
    {
      ...
      #d_j <- canolical(x=x[zInd,],y=y[zInd],tau[zInd,j],k_Liu[j],Sigma_Ridge_CEM[j])
...
SEM_Liu <-function(epsilon,x,y,beta.0,beta.old,pi.0,pi.old,sigma.0,sigma.old,maxit_EM,k_Liu,Sigma_Ridge_SEM,printout=F)
{
  ...
  for(k in 1:maxit_EM)
  {
    ...
    for (j in 1:ncol(Z)) 
    {
      ...
      #d_j <- canolical(x=x[zInd,],y=y[zInd],tau[zInd,j],k_Liu[j],Sigma_Ridge_SEM[j])
      beta_Liu <- update_Beta_Liu(x=x[zInd,],y=y[zInd],tau=tau[zInd,j],beta.k[j,],beta.old[j,],k_Liu[j],d_j)
      Beta_Liu[j,] <- beta_Liu
      sigma.Liu[j]<- Calc_Sigma(tau=tau[zInd,j],x=x[zInd,],y=y[zInd],Beta_Liu[j,])
    }
                     ...
                     ,B_true,Pi_true,sigma_true)
{
  ...
  Bias_LS_Pi_SEM     <- .biaspi(P_hat =  Pi_LS_SEM[,1:2],P_true = Pi_true[1:2])
  ...
  Bias_Ridge_Pi_SEM     <- .biaspi(P_hat =  Pi_Ridge_SEM[,1:2],P_true = Pi_true[1:2])
  ...
  Bias_Liu_Pi_SEM     <- .biaspi(P_hat =  Pi_Liu_SEM[,1:2],P_true = Pi_true[1:2])
